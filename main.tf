...
resource "google_project" "tenant_app" {
  ...
  labels = {
    ...
    gdpr_compliant = "true"
    tenant         = "webapp-team"
    workflow_test  = "dispatch-with-correct-org"
  }
}
...
# Kubernetes namespaces are now managed in shared-gke/tenant-namespaces/webapp-team.tf
# This ensures proper cross-project permissions and centralized management

# Resource quotas are now managed in shared-gke/tenant-namespaces/webapp-team.tf

# Network policies are now managed in shared-gke/tenant-namespaces/webapp-team.tf
